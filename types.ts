...
export enum AppTab {
  ...
  // Added missing members used in Layout.tsx and other features
...
// Added missing types identified from feature and service usage
...
export type AlertType = 'FALL' | 'BURN' | 'CHOKING' | 'MEDICAL' | 'NOT_SEEN' | 'UNSAFE_FOR_STAFF' | 'BEDSORES' | 'SWELLING' | 'UNABLE_TO_COMPLETE' | 'EMERGENCY' | 'OPERATIONAL' | 'SAFETY' | 'CLINICAL' | 'HSS_ENVIRONMENT' | 'BOOK_OFF' | 'AVAILABILITY' | 'UNSAFE_ENV' | 'VACATION' | 'LOA' | 'INSURANCE_Q' | 'T4_REQUEST' | 'SUPPLY_REQ' | 'PAYROLL_DISPUTE' | 'INTEGRITY_AUDIT' | 'RESTRICTION' | 'CLINICAL_COMMAND' | 'DELEGATION';

export type BlastStatus = 'PENDING' | 'SENT' | 'FAILED' | 'ACKNOWLEDGED';
...
export interface ReclamationCase {
  ...
  status: 'QUEUED' | 'READY' | 'SUBMITTED' | 'RECOVERED';
}
